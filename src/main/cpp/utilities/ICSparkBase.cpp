...
units::volt_t ICSpark::CalculateFeedforward(units::turns_per_second_squared_t accelerationTarget) {
  bool inMotionMode = GetControlType() == ControlType::kMotionProfile ||
                      GetControlType() == ControlType::kSmartMotion;
  auto velTarget = inMotionMode ? _latestMotionTarget.velocity : _velocityTarget;
  auto posTarget = inMotionMode ? _latestMotionTarget.position : _positionTarget;
  return _feedforwardStaticFriction * wpi::sgn(velTarget) + _feedforwardLinearGravity +
         _feedforwardRotationalGravity * units::math::cos(posTarget) +
         _feedforwardVelocity * velTarget + _feedforwardAcceleration * accelerationTarget;
}
...
units::volt_t ICSpark::CalcSimVoltage() {
  ...
  // store a latest copy because we can't call calculate() on the pid controller whenever we want,
  // it expects to be called at a specific frequency.
  _simVoltage = output;
